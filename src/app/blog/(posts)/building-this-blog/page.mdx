...
If you want to add a view count, Redis is a great way to implement a low weight solution that works smoothly. Using upstash makes this process so easy. First sign up and create a database. We will call this `blog views`. From here, get copy your rest token and rest URL into your environment variables file `.env.local` as `UPSTASH_REDIS_REST_TOKEN` and `UPSTASH_REDIS_REST_URL`. We also want to install the upstash dependency:

```
 npm i @upstash/redis
```

We now need to update our post data to include the view count. Firstly, extend the `PostData` to include views
...
import { Redis } from '@upstash/redis';
  ...
  // we can need to get the keys that are saved in redis
  const redisKeys = slugs.map((name) => `post_views:${name}`);

  // then we can get all these keys in one redis mget query
  const viewCounts: string[] = await redis.mget(redisKeys);

  const posts: PostData[] = await Promise.all(
    slugs.map(async (name, index) => {
      // Get the views for the specific slug
...
Note that we marked this as a client rendered component. This is crutial for being able to use hooks. We use a handy Next.js hook called [useSelectedLayoutSegments](https://nextjs.org/docs/app/api-reference/functions/use-selected-layout-segments) which lets us get the active route segment, which is the slug of the article. We can use this to find the data of the post from all the posts. Then we make a `View` component that counts and displays the current views. Note this calls a server action called `createView`. We can make that serverAction which increments the redis database at `src/app/blog/serverActions.ts`:
